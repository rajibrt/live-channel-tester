...
export async function POST(request) {
  ...
  try {
    ...
    await supabase.from("playlists").upsert(
      [{ slug: playlistSlug, name: playlistName || playlistSlug, updated_at: now }],
      { onConflict: "slug" }
    );
    ...
    for (const part of chunk(items, 200)) {
      ...
      const withPlaylistSlug = basePayload.map((x) => ({ ...x, playlist_slug: playlistSlug }));
    ...
    if (idByUrl.size < items.length) {
      for (const part of chunk(items.map((x) => x.url), 200)) {
    ...
    items.forEach((x, idx) => {
      ...
      if (id) attaches.push({ playlist_slug: playlistSlug, channel_id: id, position: idx + 1 });
    });
