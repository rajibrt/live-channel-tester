...
function pickAvatarUrl(user) {
  const meta = user?.user_metadata || {};
  return String(meta?.avatar_url || meta?.picture || meta?.profile_picture || "").trim();
}
...
export async function POST(request) {
  try {
    ...
    if (upsertRes.error) {
      return NextResponse.json({ error: upsertRes.error.message || "Failed to save profile." }, { status: 500 });
    }
    ...
    await admin.from("client_activity_events").insert({
      ...
      event_data: { via: "facebook_oauth_hash", approval_status: approvalStatus },
    });
    ...
    const redirectTo = approvalStatus === "approved" ? "/" : "/?pending=1";
