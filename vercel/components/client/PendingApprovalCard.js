...
export default function PendingApprovalCard({
  ...
  const [messengerUrl, setMessengerUrl] = useState("https://www.facebook.com/messages/t/WEBTVBD");
  ...
  async function onSubmit(event) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(payload?.error || "রিকোয়েস্ট সাবমিট করা যায়নি।");

      const nextUrl = String(payload?.messenger_url || "https://www.facebook.com/messages/t/WEBTVBD");
      ...
      if (messageText && navigator?.clipboard?.writeText) {
        try {
          ...
          setSuccess("রিকোয়েস্ট সাবমিট হয়েছে। মেসেজ কপি হয়েছে, এখন WebTVBD ইনবক্সে পেস্ট করে পাঠান।");
          showToast({
            type: "success",
            title: "মেসেজ কপি হয়েছে",
            description: "নিচের WebTVBD Facebook Inbox বাটনে ক্লিক করে মেসেজটি পেস্ট করে পাঠান।",
          });
        } catch {
          setSuccess("রিকোয়েস্ট সাবমিট হয়েছে। এখন Messenger ওপেন করে মেসেজ পাঠান।");
          showToast({
            type: "error",
            title: "অটো কপি হয়নি",
            description: "নিচের টেক্সট থেকে মেসেজ কপি করে WebTVBD Facebook Inbox-এ পাঠান।",
          });
        }
      } else {
        setSuccess("রিকোয়েস্ট সাবমিট হয়েছে। এখন Messenger ওপেন করে মেসেজ পাঠান।");
  ...
  async function copyMessageAgain() {
    ...
    try {
      ...
      showToast({
        type: "error",
        title: "কপি ব্যর্থ",
        description: "আবার চেষ্টা করুন অথবা ম্যানুয়ালি মেসেজ সিলেক্ট করে কপি করুন।",
      });
    }
  }
  ...
  return (
    <>
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: "10px", marginBottom: "12px" }}>
        <label style={{ display: "grid", gap: "6px", fontSize: "14px", color: "var(--muted-foreground)" }}>
          মোবাইল নম্বর
          <input
            type="tel"
            value={mobile}
      ...
      {error ? <p style={{ margin: "0 0 10px", color: "var(--destructive)", fontSize: "14px" }}>{error}</p> : null}
      {success ? <p style={{ margin: "0 0 10px", color: "var(--chart-2)", fontSize: "14px" }}>{success}</p> : null}
      ...
      >
        <AlertTriangle size={18} style={{ marginTop: "2px", flex: "0 0 auto" }} />
        <span>গুরুত্বপূর্ণ: WebTVBD ইনবক্সে মেসেজ না পাঠালে আপনার অ্যাকাউন্ট অ্যাক্টিভ করা হবে না।</span>
      </div>
